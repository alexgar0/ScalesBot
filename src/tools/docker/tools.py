...
def run_docker_compose_module(
    ctx: RunContext[DockerDeps], module_name: str, build: bool
) -> str:
    ...
    """
    try:
        ctx.deps.docker_compose_manager.up(module_name=module_name, build=build)
...
def get_logs_from_docker_compose_module(
    ctx: RunContext[DockerDeps], module_name: str, tail: Optional[int]
) -> str:
...
@tool()
def list_docker_compose_modules(ctx: RunContext[DockerDeps]) -> str:
